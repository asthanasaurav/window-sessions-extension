...
const DEFAULT_SETTINGS = {
  ...
  autoSaveIntervalEnabled: false,
  autoSaveIntervalMinutes: 15,
  autoSaveIntervalMaxSnapshots: 5,
  autoSaveOnCloseEnabled: false,
  autoSaveOnCloseMode: "silent",
  autoSaveOnCloseMaxSnapshots: 5,
  tabContextMenuEnabled: true,
  tabContextMenuMaxSessions: 12,
  tabContextMenuDedupeUrls: true,
};
...
export async function listManualSessions() {
  const sessions = await listSessions();
  return sessions.filter((session) => !session.isAutoSave);
}
