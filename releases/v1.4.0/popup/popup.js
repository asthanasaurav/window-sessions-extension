...
async function refresh() {
  ...
  $("headerMeta").textContent = `${sessions.length} saved session${sessions.length === 1 ? "" : "s"}`;
  renderSessionList();
...
function openDetailDialog(sessionId) {
  ...
  $("detailMeta").textContent = `${session.tabs.length} tabs · Saved ${new Date(session.updatedAt).toLocaleString()}`;
